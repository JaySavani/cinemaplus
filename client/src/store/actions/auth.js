...
export const uploadImage = (id, image) => async dispatch => {
  try {
    ...
    if (response.ok) {
      dispatch(setAlert('Image Uploaded', 'success', 1500));
    }
    if (responseData.error) {
      dispatch(setAlert(responseData.error.message, 'error', 1500));
    }
  } catch (error) {
    dispatch(setAlert(error.message, 'error', 1500));
  }
};
...
export const login = (username, password) => async dispatch => {
  try {
    ...
    if (response.ok) {
      ...
      dispatch(setAlert(`Welcome ${user.name}`, 'success', 1500));
    }
    if (responseData.error) {
...
export const register = ({
  ...
  try {
    ...
    if (response.ok) {
      ...
      dispatch(setAlert('Register Success', 'success', 1500));
    }
    if (responseData._message) {
      dispatch({ type: REGISTER_FAIL });
      dispatch(setAlert(responseData.message, 'error', 1500));
    }
  } catch (error) {
    dispatch({ type: REGISTER_FAIL });
    dispatch(setAlert(error.message, 'error', 1500));
...
export const logout = () => async dispatch => {
  try {
    ...
    if (response.ok) {
      ...
      dispatch(setAlert('LOGOUT Success', 'success', 1500));
    }
    if (responseData.error) {
      dispatch(setAlert(responseData.error.message, 'error', 1500));
    }
